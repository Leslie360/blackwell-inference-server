...
def _linear_attention_fwd_kernel(
    ...
    for prev in range(0, pid_blk):
        ...
        k_ptrs = (K_ptr + pid_b * stride_kb + pid_h * stride_kh +
                  prev_offs_n[:, None] * stride_kn + offs_dk[None, :] * stride_kd)
        k = tl.load(k_ptrs, mask=prev_offs_n[:, None] < N, other=0.0)
        v_ptrs = (V_ptr + pid_b * stride_vb + pid_h * stride_vh +
                  prev_offs_n[:, None] * stride_vn + offs_dv[None, :] * stride_vd)
    ...
    for i in range(BLOCK_N):
        ...
        q_ptrs = (Q_ptr + pid_b * stride_qb + pid_h * stride_qh +
                  pos * stride_qn + offs_dk * stride_qd)
        q = tl.load(q_ptrs, mask=pos < N, other=0.0)
        k_ptrs = (K_ptr + pid_b * stride_kb + pid_h * stride_kh +
                  pos * stride_kn + offs_dk * stride_kd)
        k_i = tl.load(k_ptrs, mask=pos < N, other=0.0)
        v_ptrs = (V_ptr + pid_b * stride_vb + pid_h * stride_vh +
                  pos * stride_vn + offs_dv * stride_vd)
        ...
        o_ptrs = (O_ptr + pid_b * stride_ob + pid_h * stride_oh +
                  pos * stride_on + offs_dv * stride_od)
...
def _decode_step_kernel(
    q_ptr, k_ptr, v_ptr, s_ptr, o_ptr,
    stride_qb, stride_qh, stride_qd,
    stride_kb, stride_kh, stride_kd,
    stride_vb, stride_vh, stride_vd,
    stride_sb, stride_sh, stride_dk, stride_dv,
    ...
    q = tl.load(q_ptr + pid_b * stride_qb + pid_h * stride_qh + offs_dk * stride_qd,
                mask=offs_dk < D_K, other=0.0)
    k = tl.load(k_ptr + pid_b * stride_kb + pid_h * stride_kh + offs_dk * stride_kd,
                mask=offs_dk < D_K, other=0.0)
    v = tl.load(v_ptr + pid_b * stride_vb + pid_h * stride_vh + offs_dv * stride_vd,
                mask=offs_dv < D_V, other=0.0)

    s_ptrs = (s_ptr + pid_b * stride_sb + pid_h * stride_sh +
              offs_dk[:, None] * stride_dk + offs_dv[None, :] * stride_dv)
    S = tl.load(s_ptrs, mask=(offs_dk[:, None] < D_K) & (offs_dv[None, :] < D_V), other=0.0)
    ...
    tl.store(o_ptr + pid_b * stride_ob + pid_h * stride_oh + offs_dv * stride_od,
             o.to(q.dtype), mask=offs_dv < D_V)
