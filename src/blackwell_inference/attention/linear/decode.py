...
def _decode_step_kernel(
    Q_ptr, K_ptr, V_ptr, S_ptr, O_ptr, LOGG_ptr,
    ...
    s_ptrs = (S_ptr + b * stride_sb + h_kv * stride_sh +
              offs_dk[:, None] * stride_sdk + offs_dv[None, :] * stride_sdv)
    s = tl.load(s_ptrs)
    ...
    for gi in range(GROUP):
        ...
        tl.store(O_ptr + b * stride_ob + h_q * stride_oh + offs_dv * stride_od,
                 o.to(O_ptr.dtype.element_ty))


def decode_step(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
                state: torch.Tensor, out: torch.Tensor | None = None,
                log_g: torch.Tensor | None = None) -> torch.Tensor:
    """
    ...
    _decode_step_kernel[grid](
        q, k, v, state, out, log_g if gated else q,
        ...
        H_KV, H_Q // H_KV,
        D_K=D_K, D_V=D_V, BLOCK_DV=BLOCK_DV, GATED=gated,
        num_warps=4,
    )
    return out
